...
declare -A lr=(["mnli"]="5e-4" ["mrpc"]="4e-4" ["qnli"]="4e-4" ["qqp"]="5e-4" ["rte"]="5e-4" ["sst2"]="5e-4" ["stsb"]="4e-4" ["cola"]="4e-4")
...
run(){
  ...
  export WANDB_NAME=roberta-base-pissa-${task_name}-r-${rank}-qkv--seed-${seed}-bs-${per_device_train_batch_size}-lr-${learning_rate}-epochs-${num_train_epochs}

  HF_ENDPOINT=https://hf-mirror.com accelerate launch --num_processes 4 --main_process_port 26698 ./run_glue_pissa.py \
