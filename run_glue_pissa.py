...
def print_pissa_parameters(model):
    classfier_params = 0
    trainable_params = 0
    pissa_params = 0
    all_param = 0
    for n, param in model.named_parameters():
        ...
        if param.requires_grad:
            if 'classifer' in n:
                classfier_params += num_params
            trainable_params += num_params
            if "lora" in n:
                pissa_params += num_params
        # print(f"{n} : {num_params:,d}\n")
    print(
        f"pissa params: {pissa_params:,d} || trainable params: {trainable_params:,d} || all params: {all_param:,d} || trainable%: {100 * trainable_params / all_param}"
    )
    print(
        f"pissa params: {pissa_params:,d} || trainable params(wo classfier): {trainable_params-classfier_params:,d} || all params: {all_param-classfier_params:,d} || trainable%: {100 * (trainable_params-classfier_params) / (all_param-classfier_params)}"
    )
    return pissa_params
...
class ModelArguments:
    ...
    mode: str = field(default="base", metadata={"help": "Which mode to use for hierachical lora. Can be 'base', 'ada' or 'me'"})
    ...
    lora_alpha: List[int] = field(
        default=128, metadata={"help": "alpha of lora"}
    )
    target_modules: Optional[List[str]] = field(
        default=None, metadata={"help": "Target modules of lora"}
    )
    lora_dropout: Optional[float] = field(
        default=0.0, metadata={"help": "Target modules of lora"}
    )
    lora_bias: str = field(
...
def main():
    ...
    with monit.section("get_PiSSA_PEFT_model"):
    ...
    pissa_params = []
    classifier_params = []
    # # 假设我们想要冻结模型的所有参数，除了 vector_z 和 classifier 相关的参数
    ...
    for name, param in model.base_model.model.named_parameters():
        ...
        if "lora" in name:
            ...
            param.requires_grad = False  # 冻结所有其他参数
            
    if accelerator.is_local_main_process:
        print_pissa_parameters(model)
                   # 遍历模型的所有参数
    ...
    optimizer = torch.optim.AdamW([{'params': pissa_params},
                                {'params': classifier_params, 'lr': training_args.learning_rate}],
    ...
    # todo : find the problem why the trainable params are so many
